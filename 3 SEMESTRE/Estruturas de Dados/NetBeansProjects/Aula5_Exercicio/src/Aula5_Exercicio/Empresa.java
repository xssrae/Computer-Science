...
//Ordenação dos Salários - Decrescente
    ...
    public static int separarSalario(Trabalhador[] vetor, int inicio, int fim) {
        ...
        while(i <= f) {
            if(vetor[i].getSalario() > pivot)i++;
            else if (vetor[f].getSalario() <= pivot)f--;
            else {
                trocar(vetor, i, f);
                i++;
                f--;
    ...
    public static void trocar(Trabalhador[] vetor, int i, int f) {
        ...
        vetor[i] = vetor[f];
        vetor[f] = troca;
        //tive problemas em fazer a troca diretamente do objeto com o getSalario(), por isso, criei mais esse método para realizar apenas isso.
    }
...
//Ordenação dos nomes - Alfabeticamente
    ...
    public static int separarNome(Trabalhador[] vetor, int inicio, int fim) {
        ...
        int i = inicio+1, f = fim;
        while(i<=f) {
            if (vetor[i].getNome().compareToIgnoreCase(pivot.getNome())<=0) i++; //se vetor[i] for menor alfabeticamente q pivo
            else if(pivot.getNome().compareToIgnoreCase(vetor[f].getNome())<0) f--;//se pivo for menor alfabeticamente q vetor[f]
            else {
                Trabalhador troca = vetor[i];
                vetor[i] = vetor[f];
                vetor[f] = troca;
